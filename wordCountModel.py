...
#    __globalGeneralWordDataDictionary={}           #Dictionary holding values for aggregate, unique, and dictionary for individual word count statistics for all articles in all subjects, broken out by subject
#    __globalIndividualWordDataDictionary={}        #Dictionary holding values for individual word count statistics for all articles across all subjects
    ...
    __earliestDateDateFormat=None                  #Variable holding the earliest date of all articles in news source in a datetime object
    __latestDateDateFormat=None                    #Variable holding the latest date of all articles in news source in a datetime object
...
# !!! IMPORTANT NOTE: When reviewing code, comments, be sure to read closely, so as to be able to discern which dictionary keys are returning values, and which dictionary keys are returning sub dictionaries !!!
    ...
    # def addTermToGlobalSourceDictionary(self, wordToAdd):
        # if wordToAdd in self.returnGlobalIndividualWordDataMemberDictionary():
            # self.__globalIndividualWordDataDictionary[wordToAdd]+=1
        # else:
            # self.__globalIndividualWordDataDictionary[wordToAdd]=1
            # self.__globalGeneralWordDataDictionary[self.__uniqueLabelKey]+=1
    ...
    # def returnGlobalGeneralWordDataDictionaryAggregateWordDataMember(self):
        # return self.__globalGeneralWordDataDictionary[self.__aggregateLabelKey]

    # def returnGlobalGeneralWordDataDictionaryUniqueWordDataMember(self):
        # return self.__globalGeneralWordDataDictionary[self.__uniqueLabelKey]

    # def returnGlobalIndividualWordDataMemberDictionary(self):
        # return self.__globalIndividualWordDataDictionary

    # def returnGlobalGeneralWordDataDictionary(self):
        # return self.__globalGeneralWordDataDictionary
    ...
    def returnGlobalSubjectValueDictionaryIndividualWordDataMemberDictionary(self, subjectValue):
        return self.__globalCollectionDictionary[self.__subjectLabelKey][subjectValue][self.__individualLabelKey]

    def returnGlobalSubjectValueDictionaryAggregateWordDataMember(self, subjectValue):
        return self.__globalCollectionDictionary[self.__subjectLabelKey][subjectValue][self.__aggregateLabelKey]

    def returnGlobalSubjectValueDictionaryUniqueWordDataMember(self, subjectValue):
    ...
    # def sortGlobalSourceWordDictionary(self):
        # sortedGlobalIndividualWordCountList={k:v for k,v in sorted(self.returnGlobalIndividualWordDataMemberDictionary().items(), key=lambda item: item[1], reverse=True)}
        # self.__globalIndividualWordDataDictionary.clear()
        # self.__globalIndividualWordDataDictionary=None
        # self.__globalIndividualWordDataDictionary=sortedGlobalIndividualWordCountList
    ...
    def calculateEarliestDate(self):
        if (self.__earliestDateDateFormat == None):
            self.__earliestDateDateFormat = self.__currentArticleDateDateFormat
        elif (self.__currentArticleDateDateFormat < self.__earliestDateDateFormat):
            self.__earliestDateDateFormat = self.__currentArticleDateDateFormat

    def calculateLatestDate(self):
        if (self.__latestDateDateFormat == None):
            self.__latestDateDateFormat = self.__currentArticleDateDateFormat
        elif (self.__currentArticleDateDateFormat > self.__latestDateDateFormat):
            self.__latestDateDateFormat = self.__currentArticleDateDateFormat
    ...
    def getEarliestDateString(self):
        self.convertEarliestDateDateToString()
        return self.__earliestDateStringFormat

    def getLatestDateString(self):
        self.convertLatestDateDateToString()
        return self.__latestDateStringFormat
